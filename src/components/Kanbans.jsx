...
export const Kanban = () => {
  ...
  const handleDrop = (item, column) => {
    ...
    if (isDropToEmptyColumn || (!clonedIssuesWithTargetId && (!columns[column] || columns[column].length === 0))) {
      const updatedColumns = { ...columns }; // копія колонки зі стейту
      // Тут я стираю взагалі таску, яку тягну за id з усіх тасок
      Object.keys(updatedColumns).forEach((key) => {
        if (updatedColumns[key] === item.id) {
          return;
        }
        if (updatedColumns[key] && Array.isArray(updatedColumns[key])) {
          updatedColumns[key] = updatedColumns[key].filter((id) => id !== parseInt(draggedId));
        }
      });
      ...
      updatedColumns[column].push(parseInt(draggedId));
    ...
    if (clonedIssuesWithTargetId) {
      const targetColumn = Object.keys(updatedColumns).find((key) => updatedColumns[key] && updatedColumns[key].includes(parseInt(targetId)));

      if (targetColumn) {
        const targetIndex = updatedColumns[targetColumn].indexOf(parseInt(targetId));
        const insertIndex = dropPosition === "after" ? targetIndex + 1 : targetIndex;

        updatedColumns[targetColumn].splice(insertIndex, 0, parseInt(draggedId));
      }
    } else {
